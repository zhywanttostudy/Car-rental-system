...
public class VehicleRentalService {
    ...
    public void reserveVehicle(JTable table, String customerId) {
        int selectedRow = table.getSelectedRow();
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(parentFrame, "请选择要预约的车辆",
        ...
        if (!"待租".equals(vehicleStatus)) {
            JOptionPane.showMessageDialog(parentFrame, "该车辆当前状态为[" + vehicleStatus + "], 无法预约",
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            try {
                ...
                if (confirm == JOptionPane.YES_OPTION) {
                    ...
                    boolean success = createReserveOrder(orderNo, customerId, vehicleNo, startDateStr, endDateStr, totalFee);

                    if (success) {
                        JOptionPane.showMessageDialog(parentFrame, "预约成功，订单号：" + orderNo,
    ...
    private boolean createReserveOrder(String orderNo, String customerNo, String vehicleNo, String startDateStr, String endDateStr, double fee) {
        ...
        try {
            ...
            // 修改 SQL 语句，添加 Ostart, Oreturn 和 Ofee 字段
            ...
            pstmt1.setString(6, "预约中");
            pstmt1.setDouble(7, fee);

            int rowsAffected = pstmt1.executeUpdate();
            if (rowsAffected != 1) {
                throw new SQLException("创建订单失败，影响行数: " + rowsAffected);
            }

            String sql2 = "UPDATE vehicle SET Vstatus='已预约' WHERE Vno=?";
            ...
            conn.commit();
            System.out.println("创建预约订单及车辆状态更新成功，订单号: " + orderNo);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            try {
                if (conn != null) {
                    conn.rollback();
                    System.err.println("事务回滚，订单未创建");
                }
            } catch (SQLException ex) {
                ex.printStackTrace();
                System.err.println("回滚失败。");
            }
            JOptionPane.showMessageDialog(parentFrame, "预约失败：" + e.getMessage(),
                    "错误", JOptionPane.ERROR_MESSAGE);
            return false;
        } finally {
            DBUtil.close(pstmt1);
    ...
    public boolean cancelReserve(String orderNo) {
        ...
        try {
            ...
            // 查询订单状态
            String sql = "SELECT Vno, Ostatus FROM orders WHERE Ono = ?";
            ...
            if (rs.next()) {
                ...
                String orderStatus = rs.getString("Ostatus").trim(); // 去除前后空格

                // 调试输出
                System.out.println("订单号: " + orderNo + ", 订单状态: " + orderStatus);

                // 检查订单状态是否为“预约中”
                if ("预约中".equals(orderStatus)) {
                    // 删除订单
                    String deleteOrderSql = "DELETE FROM orders WHERE Ono = ?";
                    pstmt2 = conn.prepareStatement(deleteOrderSql);
                    ...
                    if (rowsAffected == 1) {
                        ...
                        pstmt2 = conn.prepareStatement(updateVehicleSql);
                        pstmt2.setString(1, vehicleNo);
                        pstmt2.executeUpdate();

                        conn.commit();
                        System.out.println("取消预约成功，订单号：" + orderNo);
                        return true;
                    }
                }
            }

            JOptionPane.showMessageDialog(parentFrame, "取消预约失败，订单状态不是预约中或订单不存在",
                    "错误", JOptionPane.ERROR_MESSAGE);
            return false;
        } catch (Exception e) {
            e.printStackTrace();
            try {
                if (conn != null) {
                    conn.rollback();
                    System.err.println("事务回滚，订单未取消");
                }
            } catch (SQLException ex) {
                ex.printStackTrace();
                System.err.println("回滚失败。");
            }
            JOptionPane.showMessageDialog(parentFrame, "取消预约失败：" + e.getMessage(),
                    "错误", JOptionPane.ERROR_MESSAGE);
            return false;
        } finally {
            DBUtil.close(rs);
